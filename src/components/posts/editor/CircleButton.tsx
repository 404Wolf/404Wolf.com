...
const CircleButton = ({
    ...
    return (
        <div className="bg-gray-700 hover:brightness-90 rounded-full p-2 w-[60px] h-[60px]">
            {internalSrc && <Link href={internalSrc} target={openInNewTab ? "_blank" : undefined}>{icon}</Link>}
            {externalSrc && <a href={externalSrc} target={openInNewTab ? "_blank" : undefined}>{icon}</a>}
