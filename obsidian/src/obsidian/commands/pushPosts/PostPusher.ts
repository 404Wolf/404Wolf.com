...
export default class PostPusher {
  ...
  pushResource = async (resourceFilename: string) => {
    ...
    const resourceFileContents =
      await this.plugin.app.vault.readBinary(resourceFile);
    PostResource.create(
      this.plugin,
  ...
  pushResources = async () => {
    ...
    const thisPostResourceFileIds = new Set(
      this.post.resources.map((resource) => resource.filename),
    );
