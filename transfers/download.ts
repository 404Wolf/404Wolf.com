...
(async () => {
  ...
  types.forEach((type) => {
    ...
    if (!readdirSync(`output`).includes(`${type}s`))
      mkdirSync(`output/${type}s`);
  ...
  for (const post of posts) {
    ...
    if (!readdirSync(`${basePath}`).includes("resources"))
      mkdirSync(`${basePath}/resources`);
    ...
    for (const resource of post.resources.filter(
      (post) => !post.filename.endsWith(".md"),
    )) {
      await downloadFile(
        resource.url,
        `${basePath}/resources/${resource.filename}`,
      );
